...
public class Day2 {
	...
	public void calculateDirectDistance() {
		...
		System.out.println("Direct Distance: " + distance * depth);
	}

	public void calculateByAim() {
		int distance = 0;
		int depth = 0;
		...
		for (String step: movements) {
			...
			if (stepParts[0].equals("forward")) {
				distance += Integer.parseInt(stepParts[1]);
				depth += aim * Integer.parseInt(stepParts[1]);
			} else {
				if (stepParts[0].equals("down")) {
					aim += Integer.parseInt(stepParts[1]);
				}
				else {
					aim -= Integer.parseInt(stepParts[1]);
				}
		...
		System.out.println("Aim Based Distance: " + distance * depth);
	}
}
